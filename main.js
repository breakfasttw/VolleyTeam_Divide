(function () {
  ...
  function createBlankGroups(groupSize, minimumSlots) {
    const slots = Math.min(24, Math.max(0, minimumSlots == null ? 18 : minimumSlots));
  ...
  function bindStaticEvents() {
    ...
    elements.resultTab.addEventListener("click", () => switchView("result"));
    elements.brandHome.addEventListener("click", (event) => {
      event.preventDefault();
      switchView("setup");
    });
    elements.resetButton.addEventListener("click", confirmReset);
    elements.durationMinus.addEventListener("click", () => changeDuration(-0.5));
    elements.durationPlus.addEventListener("click", () => changeDuration(0.5));
    elements.duration.addEventListener("change", commitDuration);
    elements.eventName.addEventListener("input", () => {
      elements.eventName.value = truncateGraphemes(elements.eventName.value, 20);
    ...
    elements.groupSizeOptions.addEventListener("click", handleGroupSizeClick);
  ...
  function syncControls() {
    ...
    elements.groupSizeOptions.querySelectorAll("button").forEach((button) => {
      const selected = Number(button.dataset.groupSize) === Number(state.settings.groupSize);
  ...
  function commitDuration() {
    ...
    const next = Number.isFinite(parsed) ? clampDuration(parsed) : state.settings.duration;
    elements.duration.value = next.toFixed(1);
    ...
    if (hasAnyNames()) {
      const choice = await ui.showModal({
        ...
        actions: [
          ...
          { label: "重新分組", value: "regroup", className: "primary-button" }
        ]
      });
  ...
  function regroupMembers(members, groupSize) {
    const neededSlots = Math.min(24, Math.max(18, Math.ceil(members.length / groupSize) * groupSize));
    const groups = createBlankGroups(groupSize, neededSlots);
    members.forEach((member, index) => {
      const groupIndex = Math.floor(index / groupSize);
      const memberIndex = index % groupSize;
      groups[groupIndex].members[memberIndex] = { name: member.name, gender: member.gender };
    });
    return groups;
  }
  ...
  function handleRosterChange(event) {
    ...
    const groupIndex = Number(input.dataset.groupIndex);
    const memberIndex = Number(input.dataset.memberIndex);
    state.groups[groupIndex].members[memberIndex].gender = input.value === "M" ? "M" : "F";
    updateSummary();
    markResultStale();
  }
  ...
  function handleRosterClick(event) {
    ...
    if (button.dataset.action === "delete-group") confirmDeleteGroup(groupIndex);
  ...
  async function confirmDeleteGroup(groupIndex) {
    ...
    if (!group) return;
    const names = group.members.map((member) => member.name.trim()).filter(Boolean);
    ...
    const choice = await ui.showModal({
      ...
      actions: [
        ...
        { label: "確認刪除", value: "delete", className: "danger-button" }
      ]
    });
  ...
  async function clearPerson(groupIndex) {
    ...
    const member = state.groups[groupIndex] && state.groups[groupIndex].members[0];
    if (!member) return;
    member.name = "";
    member.gender = "F";
    renderRoster();
  ...
  function showMinimumCapacityWarning() {
    return ui.showModal({
      ...
      actions: [{ label: "知道了", value: true, className: "modal-button" }]
    });
  }
  ...
  function addUnit() {
    ...
    state.groups.push({ members: Array.from({ length: groupSize }, createBlankMember) });
    renderRoster();
    updateAddButton();
    scheduleSave();
    window.setTimeout(() => {
      const inputs = elements.roster.querySelectorAll(".member-name-input");
  ...
  function renderRoster() {
    ...
    elements.addUnit.textContent = state.settings.groupSize === 1 ? "＋ 新增人員" : "＋ 新增小組";
    updateAddButton();
  }
  ...
  function renderGroupedRoster() {
    state.groups.forEach((group, groupIndex) => {
      ...
      group.members.forEach((member, memberIndex) => {
        memberList.appendChild(createMemberRow(member, groupIndex, memberIndex, false));
      });

      const deleteButton = createDeleteButton("delete-group", groupIndex, "刪除此小組", "⌫");
  ...
  function renderSoloRoster() {
    ...
    state.groups.forEach((group, groupIndex) => {
      ...
      const deleteButton = createDeleteButton("clear-person", groupIndex, `清空第 ${groupIndex + 1} 位成員`, "⌫");
  ...
  function createMemberRow(member, groupIndex, memberIndex, solo) {
    ...
    nameInput.setAttribute("aria-label", `${solo ? `第 ${groupIndex + 1} 位` : `${groupLabel(groupIndex)}組第 ${memberIndex + 1} 位`}成員名稱`);
    ...
    genders.appendChild(createGenderChoice("F", "女", member, groupIndex, memberIndex));
    genders.appendChild(createGenderChoice("M", "男", member, groupIndex, memberIndex));
  ...
  function updateAddButton() {
    elements.addUnit.disabled = getCapacity() + state.settings.groupSize > 24;
  }
  ...
  function updateSummary() {
    ...
    elements.participantSummary.textContent = `目前共 ${members.length} 人參與，${women} 女 ${men} 男`;
  }

  function getNamedMembers() {
    return state.groups.flatMap((group) => group.members)
      .filter((member) => member.name.trim())
      .map((member) => ({ name: member.name.trim(), gender: member.gender === "M" ? "M" : "F" }));
  }
  ...
  async function startScheduling() {
    ...
    if (memberCount < 6) {
      await ui.showModal({
        ...
        title: "人數過少無法分隊",
        message: "再多揪點人！\n至少須滿 6 人",
        actions: [{ label: "好吧 ┐(´д`)┌", value: true, className: "modal-button" }]
      });
      return;
    }
    ...
    if (duplicates.length) {
      const duplicateText = duplicates.map((bucket) => {
        const first = bucket[0];
        const locations = state.settings.groupSize === 1
          ? bucket.map((item) => `第 ${item.groupIndex + 1} 位`).join("、")
          : bucket.map((item) => `${item.groupLabel}組`).join("、");
      ...
      const choice = await ui.showModal({
        ...
        actions: [
          { label: "返回修改", value: "cancel", className: "modal-button" },
          { label: "同名，要分隊", value: "continue", className: "primary-button" }
        ]
      });
    ...
    if (state.settings.groupSize > 1) {
      state.groups = state.groups.filter((group) => group.members.some((member) => member.name.trim()));
      renderRoster();
      updateSummary();
    }
    ...
    await delay(60);
    try {
      state.result = scheduler.generateSchedule({ settings: state.settings, groups: state.groups });
      state.resultStale = false;
      ...
      await ui.showModal({
        icon: "!",
        title: "暫時無法完成分隊",
        message: error && error.message ? error.message : "請檢查輸入內容後再試一次。",
        actions: [{ label: "返回檢查", value: true, className: "modal-button" }]
      });
    }
  }
  ...
  function renderResult() {
    ...
    if (!result) return;
    selectedRoundNumber = null;
    const women = result.players.filter((player) => player.gender === "F").length;
    const men = result.players.length - women;
    elements.resultSummary.textContent = `共 ${result.players.length} 人參與，${women} 女 ${men} 男`;
    elements.resultEventName.textContent = result.settings.eventName || "未命名場次";
  ...
  function renderGroupedResult(result) {
    ...
    result.groups.forEach((group, groupIndex) => {
      ...
      const namedMembers = result.players.filter((player) => player.groupIndex === groupIndex);
  ...
  function createScheduleTable(result, solo) {
    ...
    result.rounds.forEach((round) => {
      ...
      if (round.type === "placeholder") {
        ...
        if (solo) {
          frontCell.appendChild(createSoloTeam(round.front, result.players));
          backCell.appendChild(createSoloTeam(round.back, result.players));
        } else {
          frontCell.appendChild(createGroupedTeam(round.front, round, result));
          backCell.appendChild(createGroupedTeam(round.back, round, result));
          row.addEventListener("click", () => toggleRoundHighlight(round, result));
        }
      }
  ...
  function createGroupedTeam(ids, round, result) {
    ...
    ids.map((id) => result.players.find((player) => player.id === id))
      .filter(Boolean)
      .forEach((player) => {
        if (!playersByGroup.has(player.groupIndex)) playersByGroup.set(player.groupIndex, []);
        playersByGroup.get(player.groupIndex).push(player);
      });

    Array.from(playersByGroup.entries()).sort((left, right) => left[0] - right[0]).forEach(([groupIndex, sidePlayers], index) => {
      if (index) wrapper.appendChild(document.createTextNode("、"));
      const allGroupPlayers = result.players.filter((player) => player.groupIndex === groupIndex);
      const isWholeGroup = allGroupPlayers.every((player) => idSet.has(player.id)) && !round.splitGroups.includes(groupIndex);
      const shortage = allGroupPlayers.length < result.settings.groupSize ? "*" : "";
      const entry = document.createElement("span");
      entry.className = "team-entry";
      if (isWholeGroup) {
        entry.textContent = `${groupLabel(groupIndex)}${shortage}`;
      } else {
        entry.textContent = sidePlayers.map((player) => `${groupLabel(groupIndex)}${shortage}·${player.displayName}`).join("／");
      }
  ...
  function createSoloTeam(ids, players) {
    ...
    const teamPlayers = ids.map((id) => players.find((player) => player.id === id)).filter(Boolean);
  ...
  function toggleRoundHighlight(round, result) {
    selectedRoundNumber = selectedRoundNumber === round.number ? null : round.number;
    document.querySelectorAll(".schedule-row").forEach((row) => {
      row.classList.toggle("is-selected", Number(row.dataset.round) === selectedRoundNumber);
    });
  ...
  function groupsForIds(ids, players) {
    const idSet = new Set(ids);
    return new Set(players.filter((player) => idSet.has(player.id)).map((player) => player.groupIndex));
  }
  ...
  async function copyCurrentInfo() {
    ...
    await ui.showModal({
      ...
      extra: textarea,
      actions: [{ label: "完成", value: true, className: "primary-button" }]
    });
  }
  ...
  async function beginImport() {
    if (hasAnyNames()) {
      const choice = await ui.showModal({
        ...
        actions: [
          { label: "返回", value: "cancel", className: "modal-button" },
          { label: "取代現有結果", value: "replace", className: "danger-button" }
        ]
      });
  ...
  async function showImportDialog() {
    ...
    pasteButton.addEventListener("click", async () => {
      try {
        ...
        status.textContent = "瀏覽器未允許自動貼上，請長按文字框手動貼上。";
        textarea.focus();
      }
    });
    ...
    const choice = await ui.showModal({
      ...
      actions: [
        { label: "返回", value: "cancel", className: "modal-button" },
        { label: "確定匯入", value: "import", className: "primary-button" }
      ]
    });
  ...
  async function confirmReset() {
    const choice = await ui.showModal({
      ...
      actions: [
        { label: "返回", value: "cancel", className: "modal-button" },
        { label: "確認重置", value: "reset", className: "danger-button" }
      ]
    });
  ...
  function truncateGraphemes(value, maximum) {
    ...
    if (typeof Intl !== "undefined" && Intl.Segmenter) {
      const segments = Array.from(new Intl.Segmenter("zh-Hant", { granularity: "grapheme" }).segment(text), (item) => item.segment);
