...
function groupsFromSizes(sizes) {
    ...
    return sizes.map((size, groupIndex) => ({
        members: Array.from({ length: size }, () => {
            ...
            playerIndex += 1;
            return member(
                `成員${String(current + 1).padStart(2, "0")}`,
                (current + groupIndex) % 3 === 0 ? "M" : "F",
            );
        }),
    }));
}
...
function appearanceRange(result) {
    ...
    scheduledRounds(result).forEach((round) => {
        [...round.front, ...round.back].forEach((id) =>
...
(function testGroupedScheduleAndPlaceholders() {
    const input = {
        ...
        groups: groupsFromSizes(Array(9).fill(2)),
    };
    ...
    assert.deepEqual(
        first.rounds.slice(0, 3).map((round) => round.type),
        ["placeholder", "placeholder", "placeholder"],
    );
    assert.ok(
        scheduledRounds(first).every(
            (round) => round.front.length === 6 && round.back.length === 6,
        ),
    );
...
(function testSplitFallbackForUnevenGroups() {
    const input = {
        ...
        groups: groupsFromSizes([3, 3, 2]),
    };
    const result = scheduler.generateSchedule(input);
    assert.ok(
        scheduledRounds(result).every(
            (round) => round.front.length === 4 && round.back.length === 4,
        ),
    );
    assert.ok(
        scheduledRounds(result).some((round) => round.splitGroups.length > 0),
        "無法整組均分時應允許拆組",
    );
})();
...
(function testSoloSchedule() {
    const input = {
        ...
        groups: groupsFromSizes(Array(17).fill(1)),
    };
    const result = scheduler.generateSchedule(input);
...
(function testIncompleteGroupEventuallyPlays() {
    const input = {
        settings: settings(3, false),
        groups: groupsFromSizes([3, 3, 3, 3, 1]),
    };
    const result = scheduler.generateSchedule(input);
    ...
    for (const groupSize of [1, 2, 3]) {
        for (let count = 6; count <= 24; count += 1) {
            ...
            const groups = Array.from(
                { length: Math.ceil(count / groupSize) },
                () => ({
                    members: Array.from({ length: groupSize }, () => {
                        const index = cursor;
                        cursor += 1;
                        return index < count
                            ? member(`P${index}`, index % 4 === 0 ? "M" : "F")
                            : member("");
                    }),
                }),
            );
            ...
            result.rounds.forEach((round) => {
                ...
                assert.ok(
                    Math.abs(round.front.length - round.back.length) <= 1,
                );
            });
        }
    }
