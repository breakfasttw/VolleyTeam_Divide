(function () {
    ...
    function buildPlayers(groups, groupSize) {
        ...
        duplicateBuckets.forEach((bucket) => {
            ...
            bucket.forEach((player) => {
                sameGroupCounts.set(
                    player.groupIndex,
                    (sameGroupCounts.get(player.groupIndex) || 0) + 1,
                );
            });
            ...
            bucket.forEach((player) => {
                ...
                if (countInGroup > 1) {
                    const ordinal =
                        (sameGroupSeen.get(player.groupIndex) || 0) + 1;
    ...
    function generateSchedule(input) {
        ...
        const placeholderRounds = input.settings.earlyPlay
            ? Math.min(3, totalRounds)
            : 0;
        const rounds = [];
        ...
        const groupedCandidates =
            groupSize > 1 ? createIntactCandidates(players) : [];

        for (let index = placeholderRounds; index < totalRounds; index += 1) {
            const flexibleCandidates = createFlexibleCandidates(
                players,
                stats,
                index,
            );
    ...
    function createIntactCandidates(players) {
        ...
        if (total < 12) {
            ...
            subsets.forEach((frontGroups, candidateIndex) => {
                ...
                const backGroups = grouped.filter(
                    (group) => !chosen.has(group),
                );
    ...
    function groupPlayers(players) {
        ...
        return Array.from(map.entries()).map(([groupIndex, ids]) => ({
            groupIndex,
            ids,
        }));
    }
    ...
    function createFlexibleCandidates(players, stats, roundIndex) {
        ...
        for (let salt = 0; salt < 128; salt += 1) {
                ...
                selected = players
                    ...
                    .sort((left, right) => {
                        ...
                        return (
                            ...
                            stableHash(left.index, salt + roundIndex * 131) -
                                stableHash(
                                    right.index,
                                    salt + roundIndex * 131,
                                ) ||
            ...
            if (!seen.has(reversedSignature)) {
                ...
                candidates.push(
                    makeCandidate(
                        candidate.back.slice(),
                        candidate.front.slice(),
                        `fr-${roundIndex}-${salt}`,
                    ),
                );
            }
    ...
    function assignTeams(selected, frontTarget, backTarget, stats, salt) {
        ...
        selected.forEach((player) => {
            if (!byGroup.has(player.groupIndex))
                byGroup.set(player.groupIndex, []);
            byGroup.get(player.groupIndex).push(player);
        });
        const chunks = Array.from(byGroup.values()).sort(
            (left, right) =>
                stableHash(left[0].groupIndex, salt) -
                    stableHash(right[0].groupIndex, salt) ||
        ...
        chunks.forEach((chunk) => {
            ...
            chunk
                ...
                .sort(
                    (left, right) =>
                        stableHash(left.index, salt + 17) -
                            stableHash(right.index, salt + 17) ||
                        left.index - right.index,
                )
                .forEach((player) => {
                    if (front.length >= frontTarget) back.push(player.id);
                        ...
                        front.length / frontTarget <=
                        back.length / backTarget
                    )
                        front.push(player.id);
    ...
    function chooseChunkSide(
        ...
        if (Math.abs(frontFill - backFill) > 0.001)
            return frontFill < backFill ? front : back;

        const frontNeed = chunk.reduce(
            (sum, player) =>
                sum +
                stats.perPlayer[player.id].front -
                stats.perPlayer[player.id].back,
            0,
        );
    ...
    function scoreCandidate(candidate, players, stats, settings, groupSize) {
        ...
        players.forEach((player) => {
            ...
            const projectedFront =
                current.front + (front.has(player.id) ? 1 : 0);
            ...
            if (willPlay) {
                ...
                if (projectedStreak > 3) {
                    streakPenalty +=
                        (projectedStreak - 3) * (projectedStreak - 3) * 6;
                }
            } else {
                const projectedRest = current.restStreak + 1;
                if (projectedRest > 1) {
                    streakPenalty +=
                        (projectedRest - 1) * (projectedRest - 1) * 4;
                }
            }
        });

        const appearanceRange =
            Math.max(...appearanceValues) - Math.min(...appearanceValues);

        const spreadBeginners = settings.spreadBeginners === true;

        const frontBeginners = candidate.front.reduce(
            (count, id) => count + (playerById(players, id).isBeginner ? 1 : 0),
            0,
        );

        const backBeginners = candidate.back.reduce(
            (count, id) => count + (playerById(players, id).isBeginner ? 1 : 0),
            0,
        );

        const beginnerDifference = spreadBeginners
            ? Math.abs(frontBeginners - backBeginners)
            : 0;

        const frontMen = candidate.front.reduce((count, id) => {
            const player = playerById(players, id);
            const shouldCount =
                player.gender === "M" &&
                (!spreadBeginners || !player.isBeginner);

            return count + (shouldCount ? 1 : 0);
        }, 0);

        const backMen = candidate.back.reduce((count, id) => {
            const player = playerById(players, id);
            const shouldCount =
                player.gender === "M" &&
                (!spreadBeginners || !player.isBeginner);
        ...
        const genderDifference = settings.spreadMen
            ? Math.abs(frontMen - backMen)
            : 0;

        const teammateRepeat =
            pairRepeatScore(candidate.front, stats.teammates) +
            pairRepeatScore(candidate.back, stats.teammates);

        const opponentRepeat = crossRepeatScore(
            candidate.front,
            candidate.back,
            stats.opponents,
        );
    ...
    function getSplitGroups(candidate, players, groupSize) {
        ...
        players.forEach((player) => {
            if (!groupStates.has(player.groupIndex))
                groupStates.set(player.groupIndex, new Set());
            const state = front.has(player.id)
                ? "front"
                : back.has(player.id)
                  ? "back"
                  : "rest";
    ...
    function applyCandidate(candidate, stats) {
        ...
        candidate.front.forEach((frontId) => {
            candidate.back.forEach((backId) =>
                incrementMap(stats.opponents, pairKey(frontId, backId)),
            );
        });
    }
